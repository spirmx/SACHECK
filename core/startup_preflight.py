...
def _read_json_url(url: str, timeout: float) -> dict:
    ...
    request = urllib.request.Request(
        ...
        headers={"User-Agent": "SA-CHECK-Startup/2.0.1", "Accept": "application/vnd.github+json"},
    )
...
def _download_verified(url: str, expected_sha256: str, target: Path, timeout: float) -> None:
    request = urllib.request.Request(url, headers={"User-Agent": "SA-CHECK-Repair/2.0.1"})
